...
/usr/bin/git clone https://github.com/bntjah/lc-unbound/ /var/git/bntjah-unbound/

# Move the extra DNS from our Github into Cache-Domains folder
if [ -d "/var/git/bntjah-unbound/" ]; then
        mv /var/git/bntjah-unbound/* /var/git/cache-domains/
        rm -rf /var/git/bntjah-unbound/
fi
